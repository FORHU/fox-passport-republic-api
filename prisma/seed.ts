...
async function main() {
    ...
    await prisma.listingFoxxerService.deleteMany();
    await prisma.foxxerProfile.deleteMany();
    ...
    const foxxerUser = await prisma.user.create({
        data: {
            email: "foxxer@example.com",
            password: hashedPassword,
            name: "Sarah Foxxer",
            username: "sarahfoxxer",
            role: UserRole.host, // Foxxers are hosts too
            isHost: true,
            isFoxxer: true,
            isVerified: true
        }
    ...
    // 4. Create Foxxer Profile
    const foxxerProfile = await prisma.foxxerProfile.create({
        data: {
            userId: foxxerUser.id,
    ...
    const venue = await prisma.listing.create({
        data: {
            ...
            description: "A luxurious ballroom perfect for high-end weddings and corporate events. Features crystal chandeliers and a marble dance floor.",
    ...
    const equipment = await prisma.listing.create({
        data: {
            ...
            description: "Transparent premium tiffany chairs that add a touch of modern elegance to any setup.",
    ...
    const catering = await prisma.listing.create({
        data: {
            ...
            description: "A curated selection of international cuisines, prepared by award-winning chefs. Includes appetizers, main courses, and a dessert bar.",
    ...
    // 6. Link Foxxer Service to Venue
    await prisma.listingFoxxerService.create({
        data: {
            foxxerId: foxxerProfile.id,
            listingId: venue.id,
            categoryId: eventCat.id,
            serviceName: "Full Venue Styling",
            serviceDescription: "Complete transformation of the ballroom according to your theme.",
            price: 1200
        }
    });
