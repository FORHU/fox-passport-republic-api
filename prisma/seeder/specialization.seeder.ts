...
export async function seedSpecializations(prisma: PrismaClient, users: any[]) {
      ...
      rows.push(
        {
          userId: foxer.id,
          roleType: "eventFoxer",
          category: EF_CATEGORIES[i % EF_CATEGORIES.length],
          source: i % 2 === 0 ? "earned" : "declared",
        },
        {
          userId: foxer.id,
          roleType: "eventFoxer",
          category: EF_CATEGORIES[(i + 2) % EF_CATEGORIES.length],
          source: "declared",
        },
      );
    }
      ...
      rows.push(
        {
          userId: foxer.id,
          roleType: "gearFoxer",
          category: GF_CATEGORIES[i % GF_CATEGORIES.length],
          source: i % 3 === 0 ? "earned" : "declared",
        },
        {
          userId: foxer.id,
          roleType: "gearFoxer",
          category: GF_CATEGORIES[(i + 1) % GF_CATEGORIES.length],
          source: "earned",
        },
      );
    }
      ...
      rows.push(
        {
          userId: foxer.id,
          roleType: "serviceFoxer",
          category: SF_CATEGORIES[i % SF_CATEGORIES.length],
          source: i % 2 === 0 ? "earned" : "declared",
        },
        {
          userId: foxer.id,
          roleType: "serviceFoxer",
          category: SF_CATEGORIES[(i + 1) % SF_CATEGORIES.length],
          source: "declared",
        },
      );
    }
  ...
  for (const row of rows) {
    ...
    console.log(
      `  ✓ [${row.source}] ${row.roleType} / ${row.category} → ${row.userId.slice(0, 8)}…`,
    );
  }
