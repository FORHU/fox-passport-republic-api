...
describe("Central Payment & Checkout Integration Tests", () => {
  ...
  describe("Invoice Service", () => {
    it("should create an invoice and prevent duplicate invoicing", async () => {
      const data = {
        ...
        items: [
          ...
          },
          {
            ...
            sourceType: "event_service_transaction" as any,
            sourceId: "src_2",
          },
        ],
      };
      ...
      await expect(InvoiceSvc.createInvoice(data)).rejects.toThrow(
        "already associated with invoice",
      );
    });
  });
  ...
  describe("Webhook Idempotency & Voucher Redemption", () => {
    ...
    beforeAll(async () => {
      ...
      invoice = await InvoiceSvc.createInvoice({
        payerId: testUser.id,
        pricingContext: { transactionType: "event", voucherCode: voucher.code },
        items: [
          {
            amount: 1000,
    ...
    it("should confirm voucher redemption exactly once on successful payment", async () => {
      ...
      const inv = await prisma.invoice.findUnique({
        where: { id: invoice.id },
        include: { redemptions: true, payments: true },
      });
      expect(inv?.status).toBe("paid");
      expect(inv?.redemptions.length).toBe(1);
      ...
      expect(invAfter?.redemptions.length).toBe(1); // Still exactly one redemption due to idempotency
  ...
  describe("Event Checkout Consolidation", () => {
    it("should consolidate transactions and apply a voucher", async () => {
      ...
      const promo = await prisma.promotion.create({
        data: { name: "Flat 500", discountType: "fixed", discountValue: 500 },
      });
      const voucher = await prisma.voucher.create({
        data: { code: "FLAT500", promotionId: promo.id },
      });
  ...
  describe("Payouts with Absorbed Discounts", () => {
    it("should not deduct the discount from the provider payout (FoxPassport absorbs)", async () => {
      ...
      const promo = await prisma.promotion.create({
        data: { name: "Absorb", discountType: "percentage", discountValue: 10 },
      });
      const voucher = await prisma.voucher.create({
        data: { code: "ABSORB10", promotionId: promo.id },
      });
      ...
      const payouts = await prisma.payout.findMany({
        where: { sourceId: invoice.items[0].sourceId },
      });
