...
export default class VenueRepo {
  ...
  static async addBlockedDate(venueId: string, date: Date) {
    return this.retiring(
      ...
      >`UPDATE venues SET "blockedDates" = (SELECT ARRAY(SELECT DISTINCT unnest("blockedDates" || ARRAY[${date}::timestamp]) ORDER BY 1)), "updatedAt" = now() WHERE id = ${venueId} RETURNING id, "blockedDates"`.then(
        (rows) => rows[0] ?? null,
      ),
    );
  }
  ...
  static async removeBlockedDate(venueId: string, date: Date) {
    return this.retiring(
      prisma.$queryRaw<
        { id: string; blockedDates: Date[] }[]
      >`UPDATE venues SET "blockedDates" = array_remove("blockedDates", ${date}::timestamp), "updatedAt" = now() WHERE id = ${venueId} RETURNING id, "blockedDates"`.then(
        (rows) => rows[0] ?? null,
      ),
    );
  }
