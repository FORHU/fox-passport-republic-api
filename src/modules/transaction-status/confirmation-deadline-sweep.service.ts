...
export default class ConfirmationDeadlineSweepSvc {
  static async runSweep() {
    ...
    const expireOne = async (
      kind: "asset" | "service",
      row: { id: string; providerId: string; bookingId: string | null; asset?: { name: string } | null; service?: { name: string } | null },
    ) => {
      try {
        await prisma.$transaction((tx) =>
          TransactionStatusSvc.transition(tx, { id: row.id, kind, action: "expire" }),
        );
        expired++;
        ...
        failed++;
        console.error(`Confirmation deadline sweep failed for ${kind} transaction ${row.id}`, err);
      }
    };
    ...
    return { candidates: staleAssets.length + staleServices.length, expired, alreadyResolved, failed };
  }
}
