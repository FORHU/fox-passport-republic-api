...
export default class AdminSvc {
  ...
  private static async executeItemizedRefund(data: {
    ...
    const { refundId, payment, alreadyPaidOut } = await prisma.$transaction(
      async (tx) => {
        if (data.kind === "asset") {
        ...
        const row =
          data.kind === "asset"
            ? await tx.eventAssetTransaction.findUnique({
                where: { id: data.transactionId },
              })
            : await tx.eventServiceTransaction.findUnique({
                where: { id: data.transactionId },
              });
        ...
        // assumed safe.
        const countedStatuses: RefundStatus[] = [
        ...
        const remaining =
          row.agreedPrice.toNumber() -
          (alreadyRefunded._sum?.amount?.toNumber() ?? 0);
        ...
        if (!invoicePayment) {
          throw new Error(
            "No paid payment found for this item — nothing to refund",
          );
        }
    ...
    if (!payment.providerReference) {
      ...
      throw new Error(
        "Payment has no provider reference — cannot issue a Stripe refund",
      );
    }
