...
export default class EventTransactionSvc {
  ...
  static async createTransactionsFromTemplate(
    ...
    for (const item of template.templateAssets) {
      if (item.asset) {
        creations.push(
          EventTransactionRepo.createAssetTransaction({
            eventId,
            bookingId,
    ...
    for (const item of template.templateServices) {
      if (item.service) {
        creations.push(
          EventTransactionRepo.createServiceTransaction({
            eventId,
            bookingId,
    ...
    for (const item of template.templateVenues) {
      if (item.venue) {
        creations.push(
          EventTransactionRepo.createVenueTransaction({
            eventId,
            bookingId,
  ...
  static async reviewItem(
    ...
    switch (type) {
      case "asset":
        return EventTransactionRepo.updateAssetStatus(
          id,
          status,
          agreedPrice,
          providerId,
      ...
      case "service":
        return EventTransactionRepo.updateServiceStatus(
          id,
          status,
          agreedPrice,
          providerId,
      ...
      case "venue":
        return EventTransactionRepo.updateVenueStatus(
          id,
          status,
          agreedPrice,
          providerId,
