...
export default class SpecializationSvc {
  ...
  static async checkEventFoxer(organizerId: string, eventCategory: string) {
    await SpecializationSvc.maybeGrant(organizerId, RoleType.eventFoxer, eventCategory, async () => {
  ...
  static async checkServiceFoxer(serviceId: string, ownerId: string) {
    const service = await prisma.service.findUnique({ where: { id: serviceId }, select: { category: true } });
    if (!service) return;

    await SpecializationSvc.maybeGrant(ownerId, RoleType.serviceFoxer, service.category, async () => {
      const count = await prisma.serviceBooking.count({
        where: { service: { ownerId, category: service.category }, status: "completed" },
      });
  ...
  static async checkGearFoxer(assetId: string, ownerId: string) {
    const asset = await prisma.asset.findUnique({ where: { id: assetId }, select: { category: true } });
    if (!asset) return;

    await SpecializationSvc.maybeGrant(ownerId, RoleType.gearFoxer, asset.category, async () => {
      const count = await prisma.assetBooking.count({
        where: { asset: { ownerId, category: asset.category }, status: "completed" },
      });
      if (count < EARNED_THRESHOLD) return false;
  ...
  static async checkVenueFoxer(venueId: string, ownerId: string) {
    const venue = await prisma.venue.findUnique({ where: { id: venueId }, select: { category: true } });
    if (!venue) return;

    await SpecializationSvc.maybeGrant(ownerId, RoleType.venueFoxer, venue.category, async () => {
  ...
  static async getForUsers(userIds: string[]) {
    ...
    return prisma.foxerSpecialization.findMany({ where: { userId: { in: userIds } } });
  }
}
