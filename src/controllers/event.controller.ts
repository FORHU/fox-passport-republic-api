...
export default class EventController {
  ...
  static async getEventsByCategorySlug(req: Request, res: Response) {
    try {
      const schema = Joi.object({
      ...
      const events = await EventSvc.getEventsByCategorySlug(value.slug);
      return res.status(200).json({
        success: true,
        count: events.length,
        data: events,
      ...
      console.error("Get events by category slug error:", error);
      return res.status(500).json({
        success: false,
        message: error.message || "Failed to fetch events by category",
      });
    }
  }
  ...
  static async updateEventDetails(req: Request, res: Response) {
    try {
      ...
      const { userId, ...updateData } = body;
      const details = await EventSvc.updateEventDetails(
        params.id,
        userId,
        updateData
      );
