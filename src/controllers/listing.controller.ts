...
export default class ListingController {
  ...
  static async getAllListings(req: Request, res: Response) {
    try {
      const schema = Joi.object({
        ...
        type: Joi.string().valid("venue", "equipment", "catering", "service", "event").optional(),
      });
  ...
  static async createListing(req: Request, res: Response) {
    try {
      const schema = Joi.object({
        ...
        type: Joi.string().valid("venue", "equipment", "catering", "service", "event").required(),
  ...
  static async getListingsByCategorySlug(req: Request, res: Response) {
    try {
      const schema = Joi.object({
      ...
      const listings = await ListingSvc.getListingsByCategorySlug(value.slug);
      return res.status(200).json({
        success: true,
        count: listings.length,
        data: listings,
      ...
      console.error("Get listings by category slug error:", error);
      return res.status(500).json({
        success: false,
