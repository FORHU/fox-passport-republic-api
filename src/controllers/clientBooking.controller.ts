...
export default class ClientBookingController {
  ...
  static async startBooking(req: Request, res: Response) {
    try {
      const schema = Joi.object({
        ...
        type: Joi.string().valid("standard", "foxer_service").optional(),
        foxerServiceId: Joi.string().uuid().optional(),
      });
  ...
  static async selectTickets(req: Request, res: Response) {
    try {
      ...
      // PRICE VALIDATION
      const bookingForValidation = await BookingSvc.getBookingById(
        params.bookingId
      );
      const listing = bookingForValidation.listing;
      if (listing && listing.pricing && listing.pricing.length > 0) {
        const basePrice = listing.pricing[0].basePrice;
  ...
  static async addAttendees(req: Request, res: Response) {
    try {
      ...
      if (paramsError) {
        return res
          .status(400)
          .json({ success: false, message: paramsError.message });
      }
      ...
      if (bodyError) {
        return res
          .status(400)
          .json({ success: false, message: bodyError.message });
      }
  ...
  static async getMyBookings(req: Request, res: Response) {
    try {
      const querySchema = Joi.object({
        userId: Joi.string().uuid().required(),
        status: Joi.string()
          .valid("draft", "pending", "confirmed", "cancelled", "completed")
          .optional(),
      });
  ...
  static async getBookingByCode(req: Request, res: Response) {
    try {
      ...
      const booking = await BookingSvc.getBookingByConfirmationCode(
